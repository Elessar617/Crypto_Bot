...
class CoinbaseClient:
    ...
    def __init__(
        ...
        self.api_secret = api_secret if api_secret is not None else config.COINBASE_API_SECRET

        assert isinstance(self.api_key, str) and self.api_key, "API key must be a non-empty string."
        assert isinstance(self.api_secret, str) and self.api_secret, "API secret must be a non-empty string."
        ...
        try:
            ...
            self.logger.info(f"Coinbase RESTClient initialized successfully for {self.api_url} URL.")
        except Exception as e:
            self.logger.error(f"Failed to initialize Coinbase RESTClient: {e}", exc_info=True)
            raise RuntimeError(f"Coinbase RESTClient initialization failed: {e}") from e

    def _generate_client_order_id(self) -> str:
        """Generates a unique client order ID."""
        raw_id = uuid.uuid4()
        assert isinstance(raw_id, uuid.UUID), "uuid.uuid4() did not return a UUID object."
        order_id = str(raw_id)
        assert len(order_id) > 0, "Generated client_order_id is empty."
        return order_id

    def _handle_api_response(self, response: Any) -> Dict[str, Any]:
    ...
    def get_accounts(self) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            assert isinstance(response_dict, dict), "get_accounts response should be a dictionary."
            ...
            self.logger.error(f"An unexpected error occurred while retrieving accounts: {e}", exc_info=True)
            return None

    def get_product_candles(
        self, product_id: str, granularity: str, start: Optional[str] = None, end: Optional[str] = None
        ...
        try:
            ...
            assert isinstance(response_dict, dict), "get_product_candles response should be a dictionary."
            ...
            self.logger.info(f"Successfully retrieved {len(candles)} candles for {product_id}.")
            return candles
        except (HTTPError, RequestException) as e:
            self.logger.error(f"Network error retrieving candles for {product_id}: {e}", exc_info=True)
            return None
        except Exception as e:
            self.logger.error(f"An unexpected error occurred while retrieving candles for {product_id}: {e}", exc_info=True)
            return None

    def get_product_book(self, product_id: str, limit: Optional[int] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            assert isinstance(response_dict, dict), "get_product_book response should be a dictionary."
            ...
            self.logger.error(f"Network error retrieving order book for {product_id}: {e}", exc_info=True)
            return None
        except Exception as e:
            self.logger.error(f"An unexpected error occurred while retrieving order book for {product_id}: {e}", exc_info=True)
            return None
    ...
    def get_product(self, product_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            assert isinstance(product_details, dict), "get_product response should be a dictionary."
            assert 'product_id' in product_details, "'product_id' missing from product data."

            self.logger.info(f"Successfully retrieved product details for {product_id}.")
            return product_details
        except (HTTPError, RequestException) as e:
            self.logger.error(f"Network error retrieving product {product_id}: {e}", exc_info=True)
            return None
        except Exception as e:
            self.logger.error(f"An unexpected error occurred while retrieving product {product_id}: {e}", exc_info=True)
            return None

    def limit_order(self, side: str, product_id: str, size: str, price: str) -> Optional[Dict[str, Any]]:
        """Places a limit order (buy or sell)."""
        self.logger.debug(f"Attempting to place a {side} limit order for {size} {product_id} at {price}.")
        try:
            assert self.client is not None, "RESTClient not initialized."
            assert side in ["BUY", "SELL"], "Side must be 'BUY' or 'SELL'."
            assert all(isinstance(arg, str) and arg for arg in [product_id, size, price]), \
                "product_id, size, and price must be non-empty strings."

            client_order_id = self._generate_client_order_id()
            order_function = self.client.limit_order_buy if side == "BUY" else self.client.limit_order_sell
            ...
            assert isinstance(response_dict, dict), f"limit_order_{side.lower()} response should be a dictionary."

            if response_dict.get("success"):
                self.logger.info(f"Successfully placed limit {side.lower()} order {response_dict.get('order_id')}.")
            else:
                error_details = response_dict.get("error_response", {})
                message = error_details.get("message", response_dict.get("failure_reason", "Unknown error"))
                self.logger.error(f"Failed to place limit {side.lower()} order for {product_id}. Reason: {message}")

            return response_dict
        except (HTTPError, RequestException) as e:
            self.logger.error(f"Network error on limit {side.lower()} for {product_id}: {e}", exc_info=True)
            return None
        except Exception as e:
    ...
    def get_order(self, order_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            assert isinstance(response_dict, dict), "get_order response should be a dictionary."
            ...
            self.logger.error(f"Network error retrieving order {order_id}: {e}", exc_info=True)
            return None
        except Exception as e:
    ...
    def cancel_orders(self, order_ids: List[str]) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            assert self.client is not None, "RESTClient not initialized."
            assert isinstance(order_ids, list) and order_ids, "order_ids must be a non-empty list."
            ...
            self.logger.error(f"An unexpected error occurred while cancelling orders {order_ids}: {e}", exc_info=True)
            return None
